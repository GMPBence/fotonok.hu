  ...
  return (
    <div className="bg-primary h-20 px-12 py-5 flex flex-row justify-between items-center">
      ...
      <div
        ...
        onClick={toggleMenu}
      >
        <div className={`w-6 h-0.5 rounded-2xl bg-white transition-all duration-300 ${isActive ? 'rotate-45 translate-y-1.5' : ''}`}></div>
        <div className={`w-6 h-0.5 rounded-2xl bg-white transition-all duration-300 ${isActive ? 'opacity-0' : ''}`}></div>
        <div className={`w-6 h-0.5 rounded-2xl bg-white transition-all duration-300 ${isActive ? '-rotate-45 -translate-y-1.5' : ''}`}></div>
      </div>

      {/* Mobile menu that shows/hides based on isActive */}
      <div className={`absolute top-20 left-0 w-full bg-primary text-white transition-all duration-300 ${isActive ? 'block' : 'hidden'}`}>
        {/* Mobile menu content */}
        asd
      </div>
    </div>
